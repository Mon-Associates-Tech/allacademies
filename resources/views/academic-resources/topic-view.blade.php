@props(['topic', 'resources' => [], 'notes' => [], 'todos' => []])
...
<div class="academic-resource-view">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('academic-resources.group', $topic->academicSubject->academicLevel->academicGroup) }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $topic->academicSubject->academicLevel->academicGroup->name }}</a>
                </div>
            </li>
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('academic-resources.level', $topic->academicSubject->academicLevel) }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $topic->academicSubject->academicLevel->name }}</a>
                </div>
            </li>
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('academic-resources.subject', $topic->academicSubject) }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $topic->academicSubject->name }}</a>
                </div>
            </li>
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $topic->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ $topic->subtopics->count() }} subtopics available
        </p>
    </div>

    {{-- Tabs --}}
    <div x-data="{ activeTab: 'subtopics' }" class="mb-6">
        <div class="border-b border-gray-200 dark:border-gray-700">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button @click="activeTab = 'subtopics'"
                        :class="activeTab === 'subtopics' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
        ...
        {{-- Subtopics Tab Content --}}
        <div x-show="activeTab === 'subtopics'" class="mt-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse($topic->subtopics as $subtopic)
                    <a href="{{ route('academic-resources.subtopic', $subtopic) }}"
                       class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 transition-colors duration-200">
                        <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $subtopic->name }}</h5>
                        <p class="font-normal text-gray-700 dark:text-gray-400">
                            {{ $subtopic->resources->count() }} resources
                        </p>
                    </a>
                @empty
                    <div class="col-span-full text-center py-12">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No subtopics</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No subtopics available in this topic.</p>
                    </div>
                @endforelse
            </div>
        </div>
