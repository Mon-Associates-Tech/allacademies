@props(['level', 'resources' => [], 'notes' => [], 'todos' => []])
...
<div class="academic-resource-view">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('academic-resources.group', $level->academicGroup) }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $level->academicGroup->name }}</a>
                </div>
            </li>
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $level->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    {{-- Header --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $level->name }}</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ $level->academicSubjects->count() }} subjects available
        </p>
    </div>

    {{-- Tabs --}}
    <div x-data="{ activeTab: 'subjects' }" class="mb-6">
        <div class="border-b border-gray-200 dark:border-gray-700">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button @click="activeTab = 'subjects'"
                        :class="activeTab === 'subjects' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'"
        ...
        <div x-show="activeTab === 'subjects'" class="mt-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse($level->academicSubjects as $subject)
                    <a href="{{ route('academic-resources.subject', $subject) }}"
                       class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 transition-colors duration-200">
                        <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $subject->name }}</h5>
                        <p class="font-normal text-gray-700 dark:text-gray-400">
                            {{ $subject->academicTopics->count() }} topics
                        </p>
                    </a>
                @empty
                    <div class="col-span-full text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No subjects available in this level.</p>
                    </div>
                @endforelse
            </div>
        </div>
