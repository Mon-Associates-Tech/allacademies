<x-layouts.app title="My Subscriptions" page-name="Subscriptions">
    ...
    @if ($subscriptions->count())
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            @php
                $activeRegularSubscriptions = $regularSubscriptions->where('status', \App\Enums\SubscriptionStatus::PAID->value)->where('expires_at', '>', now())->count();
...
{{--                    <x-link.secondary :to="route('books.index')" class>--}}
{{--                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">--}}
...
{{--                    </x-link.secondary>--}}
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    @if($subscription['type'] === 'regular')
                                        <div class="font-medium">{{ $subscription['package']->value ?? $subscription['package'] }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm">
                                    @if($subscription['type'] === 'regular')
                                        @if($subscription['status'] === \App\Enums\SubscriptionStatus::PAID)
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                                    Paid
                                                </span>
                                        @elseif($subscription['status'] === \App\Enums\SubscriptionStatus::UNPAID)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                @if($subscription['type'] === 'regular')
                                    ...
                                    @if($subscription['status'] === \App\Enums\SubscriptionStatus::UNPAID)
                                        <form method="POST"
                                ...
                                @else
                                    @if($subscription['status'] === 'pending_payment')
                                        <button onclick="showBookPaymentDetails('{{ $subscription['reference'] }}', '{{ $subscription['amount'] }}', '{{ $subscription['book_title'] }}')"
    ...
    @else
        ...
        <div class="text-center py-12">
            ...
            <div class="mt-6 flex justify-center space-x-3">
                ...
                <x-link.secondary :to="route('students.books')">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                    </svg>
                    Browse Books
    ...
    <!-- Book Payment Modal -->
    <div id="bookPaymentModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
