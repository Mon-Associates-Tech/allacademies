<x-layouts.app>
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        @if(!$exam)
            <form method="POST" action="{{ route('examinations-hub.join.lookup') }}" class="space-y-3">
                @csrf
        ...
        @else
            <form method="POST" action="{{ route('examinations-hub.join.attempt', $exam->access_code) }}" class="space-y-3">
                @csrf
