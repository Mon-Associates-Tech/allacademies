<x-layouts.app>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7"
        ...
        <div class="flex items-center gap-0">
            ...
            <span
                class="flex items-center justify-center w-5 h-5 rounded-full bg-violet-300 text-violet-900 text-xs font-bold shrink-0">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/>
                </svg>
            </span>
                <span
                    class="text-violet-300 group-hover:text-white text-sm font-medium whitespace-nowrap transition-colors">
                Front Page
            </span>
            </a>
            {{-- Arrow --}}
            <div class="w-0 h-0 shrink-0"
                 style="border-top: 20px solid transparent; border-bottom: 20px solid transparent; border-left: 12px solid #4c1d95;"></div>
            {{-- Step 2 (active) --}}
            <div class="flex items-center gap-2.5 px-5 py-2.5 ml-0"
                ...
                <span
                    class="flex items-center justify-center w-5 h-5 rounded-full bg-white text-violet-700 text-xs font-bold shrink-0">2</span>
        ...
        <form method="POST"
            ...
            <div class="bg-white dark:bg-slate-900 overflow-hidden"
                ...
                <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
                    ...
                    <h2 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider"
                        style="letter-spacing: 0.1em;">Template Information</h2>
                </div>
                <div class="p-6 space-y-5">
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                                   style="letter-spacing: 0.08em;">Default Duration (minutes)</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                               style="letter-spacing: 0.08em;">Description</label>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="is_active" class="text-sm text-slate-700 dark:text-slate-300">Template is active and
            ...
            <div class="bg-white dark:bg-slate-900 overflow-hidden"
                ...
                <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
                    ...
                    <h2 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider"
                        style="letter-spacing: 0.1em;">Academic Scope</h2>
                </div>
                <div class="p-6 space-y-5">
                    <div class="grid md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select name="academic_group_id" x-model="selectedGroupId"
                                    @change="selectedLevelId = ''; selectedSubjectId = ''; sections.forEach(s => { s.topic_ids = []; s.subtopic_ids = []; })"
                                    class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-none focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 dark:bg-slate-800 dark:text-white transition-all"
                                    style="border-radius: 2px;">
                        ...
                        <div>
                            ...
                            <select name="academic_level_id" x-model="selectedLevelId"
                                    @change="selectedSubjectId = ''; sections.forEach(s => { s.topic_ids = []; s.subtopic_ids = []; })"
                        ...
                        <div>
                            ...
                            <select name="academic_subject_id" x-model="selectedSubjectId"
                                    @change="sections.forEach(s => { s.topic_ids = []; s.subtopic_ids = []; })"
            ...
            <div class="bg-white dark:bg-slate-900 overflow-hidden"
                ...
                <div
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-1 h-5"
                             style="background: linear-gradient(180deg, #d97706, #fbbf24); border-radius: 1px;"></div>
                        <h2 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider"
                            style="letter-spacing: 0.1em;">Sections Configuration</h2>
                        <span class="text-xs text-slate-400"
                              x-text="'(' + sections.length + ' section' + (sections.length !== 1 ? 's' : '') + ')'"></span>
                    </div>
                </div>
                ...
                <div class="p-5 space-y-4">
                    <template x-for="(section, idx) in sections" :key="section._id">
                        <div class="border border-slate-200 dark:border-slate-700 overflow-hidden relative"
                             style="border-radius: 2px;">
                            <div
                                class="px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                            ...
                            <div class="p-5 space-y-4">
                                <div class="grid sm:grid-cols-2 gap-4">
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                                            style="letter-spacing: 0.08em;">
                                    ...
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                                            style="letter-spacing: 0.08em;">Instructions</label>
                                ...
                                <div class="grid sm:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                                            style="letter-spacing: 0.08em;">Marks per Question</label>
                                        <input type="number" :name="'sections_config[' + idx + '][marks_per_question]'"
                                               x-model.number="section.marks_per_question" min="0.5" max="100"
                                               step="0.5"
                                               class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-none focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-slate-800 dark:text-white transition-all"
                                               style="border-radius: 2px;">
                                    </div>
                                ...
                                <div class="flex items-center gap-2">
                                    <input type="hidden" :name="'sections_config[' + idx + '][is_randomized]'"
                                           value="0">
                                    <input type="checkbox" :name="'sections_config[' + idx + '][is_randomized]'"
                                           :value="1"
                                           x-model="section.is_randomized"
                                           class="accent-amber-600">
                                    <label class="text-sm text-slate-700 dark:text-slate-300">Randomize question order
                                        within this section</label>
                                </div>
                                ...
                                <div x-show="availableTopics.length > 0" x-transition>
                                    ...
                                    <div
                                        class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 p-3 max-h-44 overflow-y-auto border border-slate-200 dark:border-slate-700"
                                        style="border-radius: 2px;">
                                        <template x-for="t in availableTopics" :key="t.id">
                                            <label
                                                class="flex items-start gap-2 cursor-pointer p-2 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors"
                                                style="border-radius: 2px;">
                                                <input type="checkbox"
                                                       :name="'sections_config[' + idx + '][topic_ids][]'"
                                                       :value="String(t.id)"
                                ...
                                <div x-show="section.topic_ids.length > 0 && sectionSubtopics(section).length > 0"
                                     x-transition>
                                    <label
                                        class="block text-xs font-medium text-slate-500 uppercase tracking-wider mb-2"
                                        style="letter-spacing: 0.08em;">
                                    ...
                                    <div
                                        ...
                                        <template x-for="st in sectionSubtopics(section)" :key="st.id">
                                            <label
                                                ...
                                                style="border-radius: 2px;">
                                                <input type="checkbox"
                                                       :name="'sections_config[' + idx + '][subtopic_ids][]'"
                                                       :value="String(st.id)"
                                ...
                                <div
                                    class="grid sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <div class="p-4 border border-slate-200 dark:border-slate-700"
                                         style="border-radius: 2px;">
                                        <div class="flex items-start gap-2">
                                            <input type="hidden"
                                                   :name="'sections_config[' + idx + '][insert_blank_page]'" value="0">
                                            <input type="checkbox"
                                                   :name="'sections_config[' + idx + '][insert_blank_page]'" value="1"
                                                   x-model="section.insert_blank_page"
                                                   class="mt-1 accent-amber-600">
                                            <div class="flex-1">
                                                <label
                                                    class="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">Insert
                                                    Blank Page</label>
                                                <p class="text-xs text-slate-400 mt-0.5">Adds a blank page after this
                                                    section in the final output.</p>
                                                <div x-show="section.insert_blank_page" x-transition class="mt-2">
                                                    <input type="text"
                                                           :name="'sections_config[' + idx + '][blank_page_text]'"
                                                           ...
                                                           style="border-radius: 2px;">
                                                    <p class="text-[11px] text-slate-400 mt-1">Leave blank to use the
                                                        default text above.</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="p-4 border border-slate-200 dark:border-slate-700"
                                         style="border-radius: 2px;">
                                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Attach
                                            Document</label>
                                        <p class="text-xs text-slate-400 mt-0.5 mb-2">Insert a txt, docx, pdf, or image
                                            file after this section.</p>
                                        ...
                                        <template x-if="section.attachment_original_name">
                                            <p class="text-xs text-emerald-600 mt-1.5">
                                                Currently attached: <span
                                                    x-text="section.attachment_original_name"></span>
                                        ...
                                        <input type="hidden"
                                               :name="'sections_config[' + idx + '][attachment_original_name]'"
                                               :value="section.attachment_original_name || ''">
                                        <input type="hidden"
                                               :name="'sections_config[' + idx + '][attachment_extension]'"
                                               :value="section.attachment_extension || ''">
                                        <input type="hidden" :name="'sections_config[' + idx + '][attachment_text]'"
                                               :value="section.attachment_text || ''">
                                        <input type="hidden"
                                               :name="'sections_config[' + idx + '][attachment_image_path]'"
                                               :value="section.attachment_image_path || ''">
                                        <input type="hidden"
                                               :name="'sections_config[' + idx + '][attachment_pdf_images]'"
                                               :value="JSON.stringify(section.attachment_pdf_images || [])">
                                    </div>
                                </div>
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="text-sm text-slate-600 dark:text-slate-400">
                            ...
                            <span x-text="sections.reduce((s, x) => s + (parseInt(x.question_count) || 0), 0)"></span>
                            total
                            questions,
                            <span
                                x-text="sections.reduce((s, x) => s + ((parseInt(x.question_count) || 0) * (parseFloat(x.marks_per_question) || 1)), 0).toFixed(1)"></span>
                            total marks
                        </div>
    ...
    <script>
        function templateForm() {
            ...
            @endif
                ...
                addSection() {
                    this.sections.push({
                        _id: `section-${this.sections.length}-${Date.now()}`,
                        title: '', instructions: '',
                        question_type: 'multiple_choice',
                        question_count: 10,
                        marks_per_question: 1,
