@props(['content' => '', 'pdfImages' => null, 'template' => null, 'mockExam' => null, 'subjectExam' => null, 'fontSize' => 11, 'isPdf' => false])
@php
    ...
    // attachment_pdf_images stores raw "public" disk paths (e.g. "mock-exam-attachments/uuid-page-0.jpg"),
    // unlike logo_url above which is already a usable src. NOTE: this assumes the standard Storage::url()
    // resolution — confirm against however attachment_image_path/attachment_pdf_images are already
    // rendered for section attachments elsewhere in the PDF pipeline, and swap this if it differs.
    $resolvePdfImageSrc = function (string $path) use ($isPdf) {
        return $isPdf
            ? \Illuminate\Support\Facades\Storage::disk('public')->path($path)   // local path — avoids dompdf self-fetching over HTTP
            : \Illuminate\Support\Facades\Storage::disk('public')->url($path);   // browser-usable URL for the live preview
    };
@endphp

@if( !empty($pdfImages))
    {{-- ╔══════════════════════════════════════════════════════════╗
         ║  UPLOADED PDF: rendered exactly as designed, page by page ║
         ║  Replaces the header/info-grid/content build below —     ║
         ║  the user's own document IS the front page.              ║
         ╚══════════════════════════════════════════════════════════╝ --}}
    @foreach($pdfImages as $imagePath)
        <div
            style="width: 210mm; {{ $loop->last ? '' : 'page-break-after: always;' }} {{ $isPdf ? '' : 'margin-bottom: 24px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05);' }}">
            {{-- width-only sizing (no object-fit — dompdf doesn't support it): height follows the
                 image's own aspect ratio, which matches A4 since it was rendered from a PDF page. --}}
            <img src="{{ $resolvePdfImageSrc($imagePath) }}" style="width: 210mm; display: block;">
        </div>
    @endforeach
@else
    <div class="fp-preview-container"
         style="font-family: 'Times New Roman', Times, serif; font-size: {{ $fontSize }}pt; line-height: 1.4; color: #000; background: #fff; padding: 15mm; width: 210mm; min-height: 297mm; box-sizing: border-box; margin: 0 auto; {{ $isPdf ? '' : 'box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);' }}">
                ...
                <tr>
                    @if($academicGroup)
                        <td style="padding: 8px 12px; border: 1px solid #000; width: 20%; background: #f8f8f8;">
                            <div
                                style="font-size: {{ $fontSize - 2 }}pt; text-transform: uppercase; letter-spacing: 1px; color: #555; font-weight: bold; margin-bottom: 4px;">
                                Group
                            </div>
                            <div
                                style="font-size: {{ $fontSize }}pt; font-weight: bold; color: #000;">{{ $academicGroup->name }}</div>
                        </td>
                    @endif
                    ...
                    @if($academicLevel)
                        <td style="padding: 8px 12px; border: 1px solid #000; width: 20%; background: #f8f8f8;">
                            ...
                            <div
                                style="font-size: {{ $fontSize }}pt; font-weight: bold; color: #000;">{{ $academicLevel->name }}</div>
                        </td>
                    @endif
                    ...
                    @if($academicSubject)
                        <td style="padding: 8px 12px; border: 1px solid #000; width: 20%; background: #f8f8f8;">
                            ...
                            <div
                                style="font-size: {{ $fontSize }}pt; font-weight: bold; color: #000;">{{ $academicSubject->name }}</div>
                        </td>
                    @endif
                    ...
                    @if($duration)
                        <td style="padding: 8px 12px; border: 1px solid #000; width: 20%; background: #f8f8f8;">
                            ...
                            <div
                                style="font-size: {{ $fontSize }}pt; font-weight: bold; color: #000;">{{ $durationText }}</div>
                        </td>
                    @endif
                    ...
                    <td style="padding: 8px 12px; border: 1px solid #000; width: 20%; background: #f8f8f8;">
                        ...
                        <div
                            style="font-size: {{ $fontSize }}pt; font-weight: bold; color: #000;">{{ number_format($totalMarks, 1) }}</div>
                    </td>
                </tr>
            </table>
        </div>
        ...
        @if($content)
            <div class="fp-richtext"
                 style="margin-bottom: 16px; text-align: left; color: #111; line-height: 1.6;">                {!! $content !!}
            </div>
        @endif
    </div>
@endif
