                            ...
                            <div class="flex items-center space-x-1">
                                ...
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                         ...
                                         x-transition
                                         class="absolute right-0 mt-1 w-56 bg-white rounded-md shadow-lg border border-gray-200 z-10">
                                        <div class="py-1">
                                            ...
                                            @if ($team->owner->isNot($user))
                                                <button x-data="{}"
                                                        x-on:click="$store.deleteForm.show('Leave Team', 'Are you sure you want to leave {{ $team->name }}?', '{{ route('teams.members.destroy', ['team' => $team, 'member' => $user]) }}', 'Leave')"
                                            ...
                                            @if ($team->owner->is($user) && ! $team->is_personal && $team->subscriptions_count === 0)
                                                <button x-data="{}"
                                                        x-on:click="$store.deleteForm.show('Delete Team', 'Are you sure you want to delete {{ $team->name }}? This action cannot be undone.', '{{ route('teams.destroy', ['team' => $team]) }}', 'Delete')"
                                                        class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                                                    <svg class="inline w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
