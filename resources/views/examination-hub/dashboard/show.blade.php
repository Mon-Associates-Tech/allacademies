<x-layouts.app>
    ...
    <div id="exam-show-printable" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7"
        ...
        <div class="space-y-4">
            ...
            {{-- ── EMAIL INVITATIONS ── --}}
            <div x-data="{ open: false }" class="bg-white dark:bg-slate-900 overflow-hidden" style="border-radius: 2px; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 1px 6px rgba(0,0,0,0.04);">
                <button @click="open = !open" class="w-full px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors text-left">
                    <div class="flex items-center gap-2">
                ...
                <div x-show="open" x-cloak class="p-5 space-y-5">
                    <div class="flex items-start gap-2.5 p-3.5 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900" style="border-radius: 2px;">
                    ...
                    <form action="{{ route('examination-hub.exams.send-invitations', $exam) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #1d4ed8, #2563eb); box-shadow: 0 2px 8px rgba(37,99,235,0.25);">
                    ...
                    <form action="{{ route('examination-hub.exams.reminder-settings', $exam) }}" method="POST" class="space-y-3">
                        @csrf
                        <div class="flex items-center gap-3 px-3.5 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800" style="border-radius: 2px;">
                            <input type="checkbox" name="send_reminders" id="send_reminders" value="1" {{ $exam->send_reminders ? 'checked' : '' }} class="w-4 h-4 rounded text-amber-600 border-slate-300 dark:border-slate-600 focus:ring-amber-500">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="reminder_datetime" value="{{ $exam->reminder_datetime?->format('Y-m-d\TH:i') }}" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                        </div>

                        <button type="submit" class="w-full py-2 text-sm font-medium text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" style="border-radius: 2px;">
                            Save Reminder Settings
                        </button>
                    </form>

                    @if($exam->reminder_sent)
                        <div class="flex items-center gap-2 px-3.5 py-2.5 border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30" style="border-radius: 2px;">
                    ...
                    <form action="{{ route('examination-hub.exams.send-reminder', $exam) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #b45309, #d97706); box-shadow: 0 2px 8px rgba(180,83,9,0.25);">
            ...
            <div x-data="{ open: false }" class="bg-white dark:bg-slate-900 overflow-hidden" style="border-radius: 2px; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 1px 6px rgba(0,0,0,0.04);">
                <button @click="open = !open" class="w-full px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors text-left">
                    ...
                    <div class="flex items-center gap-3">
                        <span class="text-xs px-2 py-1 font-medium text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-800" style="border-radius: 2px;">{{ ucfirst($exam->participant_mode) }}</span>
                        <svg class="w-4 h-4 text-slate-400 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex gap-2">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="participant_mode" value="general" {{ $exam->participant_mode === 'general' ? 'checked' : '' }} class="w-4 h-4 text-amber-600 border-slate-300 focus:ring-amber-500">
                                    <span class="ml-2 text-sm text-slate-700 dark:text-slate-300">General</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="participant_mode" value="configured" {{ $exam->participant_mode === 'configured' ? 'checked' : '' }} class="w-4 h-4 text-amber-600 border-slate-300 focus:ring-amber-500">
                                    <span class="ml-2 text-sm text-slate-700 dark:text-slate-300">Configured</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="participant_mode" value="both" {{ $exam->participant_mode === 'both' ? 'checked' : '' }} class="w-4 h-4 text-amber-600 border-slate-300 focus:ring-amber-500">
                ...
                <div x-show="open" x-cloak class="p-5 space-y-4">
                    <form action="{{ route('examination-hub.exams.proctoring-settings', $exam) }}" method="POST" class="space-y-4">
                        ...
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="proctoring_enabled" value="1" {{ $exam->proctoring_enabled ? 'checked' : '' }} class="w-4 h-4 rounded text-amber-600 border-slate-300 dark:border-slate-600 focus:ring-amber-500">
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Enable Proctoring</span>
                        </label>

                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="auto_submit_on_violation" value="1" {{ $exam->auto_submit_on_violation ? 'checked' : '' }} class="w-4 h-4 rounded text-amber-600 border-slate-300 dark:border-slate-600 focus:ring-amber-500">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" min="0" max="100" name="auto_submit_high_severity_threshold" value="{{ old('auto_submit_high_severity_threshold', $exam->auto_submit_high_severity_threshold ?? 2) }}" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                            </div>
                            <div>
                                ...
                                <input type="number" min="0" max="100" name="auto_submit_medium_severity_threshold" value="{{ old('auto_submit_medium_severity_threshold', $exam->auto_submit_medium_severity_threshold ?? 5) }}" class="w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                            </div>
                        </div>
                        ...
                        <div class="border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-3 space-y-2" style="border-radius: 2px;">
                            <p class="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider" style="font-size: 10px; letter-spacing: 0.08em;">Violation Types</p>
                            <p class="text-xs text-slate-600 dark:text-slate-400"><span class="font-semibold text-red-700 dark:text-red-400">High:</span> exam exit, multiple faces, face mismatch.</p>
                            <p class="text-xs text-slate-600 dark:text-slate-400"><span class="font-semibold text-amber-700 dark:text-amber-400">Medium:</span> tab switch, window blur, fullscreen exit, no face detected.</p>
                            <p class="text-xs text-slate-600 dark:text-slate-400"><span class="font-semibold text-blue-700 dark:text-blue-400">Low:</span> copy attempt, paste attempt, keyboard shortcut, right click/context menu.</p>
                        </div>
                    ...
                    <form action="{{ route('examination-hub.exams.violation-settings', $exam) }}" method="POST" class="space-y-1">
                        ...
                        @foreach($violationMeta as $key => $meta)
                            <label class="flex items-center justify-between py-2.5 border-b border-slate-50 dark:border-slate-800 last:border-0 cursor-pointer group">
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center text-xs font-semibold px-2 py-0.5 border {{ $severityColors[$meta['severity']] }}" style="border-radius: 2px;">{{ ucfirst($meta['severity']) }}</span>
                                ...
                                <input type="checkbox" name="violations[{{ $key }}]" value="1" {{ ($resolvedViolations[$key] ?? true) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-slate-300 dark:border-slate-600 focus:ring-indigo-500">
                            </label>
                        @endforeach
                        <div class="pt-3">
                            <button type="submit" class="w-full py-2 text-sm font-medium text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #4f46e5, #6366f1); box-shadow: 0 2px 8px rgba(99,102,241,0.25);">
                        ...
                        <div class="flex items-center justify-between py-2">
                            ...
                            <dd>
                                @if($exam->canShowResults())
                                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800" style="border-radius: 2px;">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                        Yes
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700" style="border-radius: 2px;">
                    ...
                    @if($exam->result_visibility === 'manual_release')
                        <form action="{{ route('examination-hub.exams.toggle-results', $exam) }}" method="POST">
                            ...
                            @if($exam->results_released)
                                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #dc2626, #ef4444); box-shadow: 0 2px 8px rgba(220,38,38,0.25);">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                    Hide Results from Participants
                                </button>
                            @else
                                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #059669, #10b981); box-shadow: 0 2px 8px rgba(5,150,105,0.25);">
                    ...
                    <form action="{{ route('examination-hub.participants.configured.store', $exam) }}" method="POST" class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input name="first_name" placeholder="e.g. John" required class="w-full px-3 py-2.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                            </div>
                            <div>
                                ...
                                <input name="last_name" placeholder="e.g. Mensah" required class="w-full px-3 py-2.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                            </div>
                        </div>
                        <div>
                            ...
                            <input name="email" type="email" placeholder="e.g. john@example.com" required class="w-full px-3 py-2.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                        </div>
                        <div>
                            ...
                            <input name="unique_code" placeholder="e.g. STU-2024-001" class="w-full px-3 py-2.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                        </div>
                        <button type="submit" class="w-full py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #0f172a, #1e293b); box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                            Add Participant
                        </button>
                    </form>
                </div>
            ...
            <div x-data="{ open: false }" class="bg-white dark:bg-slate-900 overflow-hidden" style="border-radius: 2px; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 1px 6px rgba(0,0,0,0.04);">
                ...
                <div x-show="open" x-cloak class="p-5">
                    <div class="flex items-center gap-2 px-3 py-2 mb-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700" style="border-radius: 2px;">
                    ...
                    <form action="{{ route('examination-hub.participants.configured.import', $exam) }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="file" name="participants_csv" accept=".csv" required class="w-full text-sm text-slate-600 dark:text-slate-400 file:mr-3 file:py-2 file:px-4 file:border-0 file:text-xs file:font-semibold file:text-white file:cursor-pointer border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 cursor-pointer" style="border-radius: 2px; --tw-file-border-radius: 2px;" onchange="this.parentElement.querySelector('style') || (()=>{ const s=document.createElement('style'); s.textContent='input[type=file]::file-selector-button{background:linear-gradient(135deg,#1e293b,#334155);border-radius:2px;}'; this.parentElement.appendChild(s); })()">
                        </div>
                        <button type="submit" class="w-full py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #0369a1, #0284c7); box-shadow: 0 2px 8px rgba(3,105,161,0.25);">
                            Import from CSV
                        </button>
                    </form>
                </div>
                ...
                <div x-show="open" x-cloak class="p-5">
                    <form action="{{ route('examination-hub.participants.configured.import-group', $exam) }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="participant_group_id" required class="w-full px-3 py-2.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all" style="border-radius: 2px;">
                        ...
                        <button type="submit" class="w-full py-2.5 text-sm font-semibold text-white transition-all" style="border-radius: 2px; background: linear-gradient(135deg, #7c3aed, #a78bfa); box-shadow: 0 2px 8px rgba(124,58,237,0.25);">
            ...
            <div class="no-print" x-data="{ open: true }" class="bg-white dark:bg-slate-900 overflow-hidden" style="border-radius: 2px; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 1px 6px rgba(0,0,0,0.04);">
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <span class="text-xs px-2 py-1 font-semibold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800" style="border-radius: 2px;">{{ $configuredCount }} total</span>
                        @if($configuredCount > 0)
                            <form method="POST" action="{{ route('examination-hub.participants.configured.destroy-all', $exam) }}" onsubmit="return confirm('Remove all configured participants from this exam?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center gap-2 px-3 py-2 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900 transition-all" style="border-radius: 2px;">
                                ...
                                <tr class="hover:bg-amber-50/40 dark:hover:bg-slate-800/40 transition-colors">
                                    ...
                                    <td class="px-6 py-3.5">
                                        @if($participant->unique_code)
                                            <span class="font-mono text-xs px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300" style="border-radius: 2px;">{{ $participant->unique_code }}</span>
                                        @else
                                    ...
                                    <td class="px-6 py-3.5">
                                        <div class="flex items-center justify-end gap-2">
                                            <form method="POST" action="{{ route('examination-hub.participants.configured.toggle', [$exam, $participant]) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium border transition-colors {{ $participant->is_active ? 'text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800 hover:bg-amber-50 dark:hover:bg-amber-950/30' : 'text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/30' }}" style="border-radius: 2px;" title="{{ $participant->is_active ? 'Deactivate' : 'Activate' }}">
                                                    @if($participant->is_active)
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                                                        Deactivate
                                                    @else
                                            ...
                                            <a href="{{ route('examination-hub.participants.configured.edit-form', [$exam, $participant]) }}" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800 hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors" style="border-radius: 2px;" title="Edit participant">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                Edit
                                            </a>

                                            <form method="POST" action="{{ route('examination-hub.participants.configured.destroy', [$exam, $participant]) }}" onsubmit="return confirm('Remove {{ addslashes($participant->name) }} from this exam?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors" style="border-radius: 2px;" title="Remove participant">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                    Remove
                                                </button>
                                            </form>
