    ...
    <div class="relative {{ $heightClass }}">
        ...
        <div wire:ignore x-data="{
            ...
            init() {
                ...
                console.log('[LineChart] init payload:', payload);
                ...
                const tryCreate = (attempt = 0) => {
                    if (!window.ChartHelpers) {
                        if (attempt === 0) console.warn('[LineChart] ChartHelpers not found. Waiting for bundle to attach...');
                        if (attempt < 10) {
                            return setTimeout(() => tryCreate(attempt + 1), 100);
                        } else {
                            console.warn('[LineChart] ChartHelpers still missing after retries. Is resources/js/charts/helpers.js imported and built?');
                            return;
                        }
                    }
                    this.$nextTick(() => {
                        try {
                            ...
                            if (existing) {
                                existing.destroy();
                                console.log('[LineChart] existing Chart.js instance destroyed (registry)');
                            }
                            ...
                            if (this.chart && window.ChartHelpers) {
                                ...
                                this.chart = null;
                                console.log('[LineChart] previous chart destroyed before re-create');
                            }
                            if (window.ChartHelpers && payload?.data) {
                                ...
                                this.created = true;
                                console.log('[LineChart] chart created:', this.chart?.config?.type);
                            } else {
                                console.warn('[LineChart] Skipping chart creation (helpers or data missing).');
                            }
                        } catch (e) {
                            console.error('[LineChart] error creating chart:', e);
                        }
                    });
                };
                tryCreate();
                this.$watch('$store.darkMode.on', (v) => {
                    ...
                    console.log('[LineChart] dark mode changed:', v);
            ...
            destroy() {
                if (this.chart && window.ChartHelpers) {
                    window.ChartHelpers.destroyChart(this.chart);
                    console.log('[LineChart] chart destroyed');
                }
                this.chart = null;
            }
