<div class="space-y-2">
    ...
    @if ($errors->has('title') ||  $errors->has('date') || $errors->has('start') || $errors->has('end') || $errors->has('instructions') || $errors->has('examiners'))
        <span class="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-red-100 text-red-800">Heading Inputs Error!</span>
    @endif
    <div class="rounded-xl bg-gray-200 p-2">
        @if($package)
            <div class="grid grid-cols-4 gap-3">
                <div>
                    <input type="radio" wire:model="heading_type" id="0" value="0" class="peer hidden" />
                    <label for="0" class="block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-primary-600 peer-checked:font-bold peer-checked:text-white text-sm font-medium">
                        Choose Template
                ...
                <div>   
                    <input type="radio" name="heading_type" wire:model="heading_type" id="1" value="1" class="peer hidden" required="required" />
                ...
                <div>
                    <input type="radio" name="heading_type" wire:model="heading_type" id="2" value="2" class="peer hidden" />
                ...
                <div>
                    <input type="radio"  name="heading_type" wire:model="heading_type" id="3" value="3" class="peer hidden" />
        ...
        @if($heading_type != null)
            <div class="pt-4">
                <div x-data="{ preview: false}">
                    ...
                    <div x-model.lazy="down" x-show="!preview" id="write-heading" class="block px-4 py-2 text-gray-700 shadow-sm border border-gray-300 w-full rounded-b-lg">
                        @if($heading_type == "3")
                            <div class="bg-blue-50 border border-blue-200 text-sm text-blue-600 rounded-md p-4 mb-4" role="alert">
                                <span class="font-bold">Note:</span> Make sure to add institution and department name, and logo under <span class="font-bold">Edit Team</span> section.
                            </div>
                        @elseif($heading_type == "2")
                        <div class="bg-blue-50 border border-blue-200 text-sm text-blue-600 rounded-md p-4 mb-4" role="alert">
                            <span class="font-bold">Note:</span> Make sure to add institution name under <span class="font-bold">Edit Team</span> section.
                        </div>
                        @endif
                    ...
                    <div x-show="preview" class="px-4 py-2 text-gray-700 focus:outline-none focus:border-gray-700 bg-white border border-gray-300 rounded-b-lg">
                        ...
                        @elseif ($heading_type == "2")
                            ...
                            <div class="text-center font-bold">
                                <h2>{{ Str::upper(is_null($metaData) ? null : $metaData->meta['school'] ?? 'INSTITUTION NAME') }}</h2>
