@php
    ...
    if ($package) {
        ...
        $school = is_null($metaData) ? 'SCHOOL' : Str::ucfirst($metaData['school']);
        $faculty = is_null($metaData) ? 'FACULTY' : Str::ucfirst($metaData['faculty']);
        $college = is_null($metaData) ? 'COLLEGE' : Str::ucfirst($metaData['college']);
        $department = is_null($metaData) ? 'DEPARTMENT' : Str::ucfirst($metaData['department']);
    }
    ...
    <div class="rounded-xl bg-gray-200 p-2">
        @if ($package)
            <div class="grid grid-cols-4 gap-3">
                <div>
                    <input type="radio" name="heading_type" wire:model="heading_type" id="1" value="basic"
                        class="peer hidden" required="required" />
                    <label for="1"
                        class="block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-primary-600 peer-checked:font-bold peer-checked:text-white text-sm font-medium">Basic</label>
                </div>

                <div>
                    <input type="radio" name="heading_type" wire:model="heading_type" id="2"
                        value="institutional" class="peer hidden" />
                    <label for="2"
                        class="block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-primary-600 peer-checked:font-bold peer-checked:text-white text-sm font-medium">Institutional</label>
                </div>

                <div>
                    <input type="radio" name="heading_type" wire:model="heading_type" id="3"
                        value="institutional_advanced" class="peer hidden" />
                    <label for="3"
                        class="block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-primary-600 peer-checked:font-bold peer-checked:text-white text-sm font-medium">Advanced
                        - Institutional</label>
                </div>
            </div>
        @elseif(!$package)
            ...
            <div x-data="{ preview: false }">
                <div class="bg-white border-x border-t border-gray-300 rounded-t-lg">
                    <div class="text-xs pl-3">
                        <button x-bind:class="!preview && 'font-medium tracking-wide'" x-on:click="preview = false"
                            type="button" class="py-2 px-1">Write</button>
                        <button x-bind:class="preview && 'font-medium tracking-wide'" x-on:click="preview = true"
                            type="button" class="py-2 px-1">Preview</button>
                    </div>
                </div>
                <div x-show="!preview"
                    class="block px-4 py-2 text-gray-700 shadow-sm border border-gray-300 w-full rounded-b-lg">
                ...
                <div x-show="preview"
                    class="px-4 py-2 text-gray-700 focus:outline-none focus:border-gray-700 bg-white border border-gray-300 rounded-b-lg">
                    @if ($heading_type == 'basic' || $package == null)
                        {{-- heading 1 - individual --}}
                        @if (!$package)
                            <div class="flex justify-center">
                                <img src="{{ URL::to('/img/logo.png') }}" class="w-20" alt=""
                                    onerror="this.style.display='none'" />
                            </div>
                        @endif
                        <div class="text-center">
                            @if ($package)
                                <h3 class="font-bold">{{ $institution }}</h3>
                            @endif
                    ...
                    @elseif ($heading_type == 'institutional')
                        ...
                        <div class="flex justify-center">
                            <img src="{{ $logo_path }}" class="w-20" alt=""
                                onerror="this.style.display='none'" />
                        </div>
                        <div class="text-center">
                            <h3 class="font-bold">{{ $institution }}</h3>
                        ...
                        <div class="font-bold mt-4">
                            <p>Date: {{ $exam_date }}</p>
                            <p>Duration: {{ $start_time }} <span class="mr-1 ml-1"> — </span> {{ $end_time }}
                                </h3>
                        </div>
                        <p cLass="font-bold underline mt-4">Exams Instructions</p>
                        <P class="text-justify mb-8">{{ $instructions }}</P>
                    @elseif($heading_type == 'institutional_advanced')
                        ...
                        <div class="text-center">
                            ...
                            <h3>{{ $academic_group }} <span class="mr-4"></span> {{ $academic_level }}</h3>
                            <h3>{{ $exam_title }}</h3>
                            <h3>{{ $subject_code . ' ' . $subject_name }} <span class="mr-4"></span>
                                {{ $exam_date }}</h3>
                            <h3>{{ $start_time }} <span class="mr-1 ml-1"> — </span> {{ $end_time }}</h3>
                        </div>
