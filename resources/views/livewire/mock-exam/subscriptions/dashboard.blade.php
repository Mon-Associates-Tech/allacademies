<div>
    ...
    @if($activeTab === 'subscriptions')
                ...
                @if($calculatedPrice > 0)
                    <div class="rounded-md border border-indigo-200 bg-indigo-50 p-4">
                        <div class="text-lg font-semibold text-indigo-700">
                            Total: GHS {{ number_format($calculatedPrice, 2) }}
                        </div>

                        @if($selectedPricingTier)
                            <div class="mt-1 text-sm text-indigo-700">
                                Pricing Tier: {{ $selectedPricingTier->name }}
                            </div>
                        @endif

                        <div class="mt-1 text-sm text-indigo-600">
                            @if($type === 'online')
                                {{ count($selectedSubjectIds) }} subject(s)
                                × {{ $participantCount }} participant(s)
                            @else
                                {{ count($selectedSubjectIds) }} subject(s) flat rate
                            @endif
                        </div>
                    </div>
                @endif
        ...
        <div class="grid gap-4">
            @forelse($mockSubscriptions as $sub)
    ...
    @if($activeTab === 'results')
        ...
        @if($selectedExamId && $mockSubmissions->count())
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <table class="w-full text-sm">
                    ...
                    @foreach($mockSubmissions as $submission)
        ...
        {{-- Score Editor Modal --}}
        @if($showScoreEditor && $editingMockSubmission)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                <div class="bg-white dark:bg-gray-800 rounded-xl w-full max-w-2xl shadow-xl max-h-[90vh] flex flex-col">
                    <div class="p-5 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="font-semibold text-gray-800 dark:text-gray-100">Edit Scores — {{ $editingMockSubmission->getParticipantName() }}</h3>
                    </div>
                    <div class="p-5 overflow-y-auto flex-1">
                        @foreach($editingMockSubmission->assignment->questions as $question)
