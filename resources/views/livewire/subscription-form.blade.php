            ...
            <div class="overflow-hidden rounded-lg border border-gray-300 bg-white">
                ...
                @foreach ($academicGroups as $groupIndex => $academicGroup)
                    <li wire:key="academic_group_{{ $academicGroup['id'] }}" class="p-3">
                        <p wire:click="$toggle('academicGroups.{{ $groupIndex }}.is_open')" role="button" class="flex items-center justify-between cursor-pointer">
                        ...
                        <div class="mt-3 pt-3 px-3 border-t border-gray-300">
                            <ul role="list" class="divide-y divide-gray-200">
                                @foreach ($academicGroup['academic_levels'] as $levelIndex => $academicLevel)
                                    <li wire:key="academic_level_{{ $academicLevel['id'] }}" class="py-3 first:pt-0 last:pb-0">
                                        <p wire:click="$toggle('academicGroups.{{ $groupIndex }}.academic_levels.{{ $levelIndex }}.is_open')" role="button" class="flex items-center justify-between cursor-pointer">
                                            ...
                                            <fieldset>
                                                <div class="space-y-5">
                                                    @foreach ($academicLevel['academic_subjects'] as $subjectIndex => $academicSubject)
                                                        <div wire:key="academic_subject_{{ $academicSubject['id'] }}" class="relative flex items-start">
                                                            <div class="flex h-6 items-center">
                                                                <input value="{{ $academicSubject['id'] }}" wire:model="academicSubjects" name="academic_subject_ids[]" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                                                            </div>
                                                            <div class="ml-3 text-sm leading-6">
                                                                <label class="font-medium text-gray-900">{{ $academicSubject['name'] }}</label>
                                                                <span class="text-gray-500"><span>{{ $academicSubject['code'] }}</span></span>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
