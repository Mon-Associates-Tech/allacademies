<div class="flex h-screen bg-gray-50" x-data="chatInterface()">
    ...
    <div class="w-80 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div class="p-4 border-b border-gray-200">
            <div class="flex items-center justify-between mb-3">
                ...
                <button
                    wire:click="$set('showCreateGroup', true)"
    ...
    <div class="flex-1 flex flex-col">
        @if($selectedChatGroup)
            ...
            <div class="p-4 bg-white border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        <div class="w-10 h-10 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center">
                            <span class="text-white font-medium text-sm">{{ substr($selectedChatGroup->name, 0, 1) }}</span>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-900">{{ $selectedChatGroup->name }}</h3>
                            <p class="text-sm text-gray-500">
                                {{ $selectedChatGroup->members()->count() }} member{{ $selectedChatGroup->members()->count() !== 1 ? 's' : '' }}
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div
                                ...
                                <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Group Info</a>
                                <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Mute Notifications</a>
                                @if($selectedChatGroup->created_by === auth()->id())
                                    <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50">Delete Group</a>
                                @else
                                    <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50">Leave Group</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4"
                ...
                @foreach($messages as $message)
                    <div class="flex {{ $message->user_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="flex max-w-xs lg:max-w-md {{ $message->user_id === auth()->id() ? 'flex-row-reverse' : 'flex-row' }} items-end space-x-2">
                            <!-- Avatar -->
                            @if($message->user_id !== auth()->id())
                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center flex-shrink-0">
                                    <span class="text-white text-xs font-medium">{{ substr($message->user->name, 0, 1) }}</span>
                                </div>
                            @endif
                            ...
                            <div class="relative group">
                                ...
                                <div class="px-4 py-2 rounded-2xl {{ $message->user_id === auth()->id() ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-900' }}">
                                        ...
                                        @if($message->attachments && $message->attachments->count() > 0)
                                            <div class="mt-2 space-y-2">
                                                @foreach($message->attachments as $attachment)
                                                    ...
                                                    @else
                                                        <div class="flex items-center space-x-2 p-2 bg-black bg-opacity-10 rounded-lg">
                                ...
                                @if(!$message->is_deleted && $message->message_type !== 'system')
                                    <div class="absolute top-0 {{ $message->user_id === auth()->id() ? 'left-0 transform -translate-x-full' : 'right-0 transform translate-x-full' }} -translate-y-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div class="p-4 bg-white border-t border-gray-200">
                <!-- Reply Preview -->
                @if($replyToMessage)
                    <div class="flex items-center justify-between mb-3 p-3 bg-gray-50 rounded-lg border-l-4 border-l-blue-500">
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-medium text-gray-700">Replying to {{ $replyToMessage->user->name }}</p>
                    ...
                    <div class="mb-3 flex flex-wrap gap-2">
                        @foreach($attachments as $index => $attachment)
                            <div class="relative bg-gray-100 rounded-lg p-3 flex items-center space-x-2 max-w-xs">
                                ...
                                <span class="text-sm text-gray-700 truncate flex-1">{{ $attachment->getClientOriginalName() }}</span>
                                <button
                                    type="button"
                ...
                <form wire:submit.prevent="sendMessage" class="flex items-end space-x-3">
                    <!-- File Upload -->
                    <div class="flex-shrink-0">
                        <label for="file-upload" class="cursor-pointer p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path>
                            </svg>
                        </label>
        ...
        @else
            ...
            <div class="flex-1 flex items-center justify-center bg-gray-50">
                <div class="text-center max-w-md">
                    ...
                    <p class="text-gray-600 mb-6">Select a conversation to start messaging or create a new group to get started.</p>
                    <button
                        wire:click="$set('showCreateGroup', true)"
    ...
    <!-- Members Modal -->
    @if($selectedChatGroup)
        <div x-show="showMembersModal" x-data="{ showMembersModal: false }" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div
                ...
                <div class="p-6">
                    <livewire:chats.group-members-manager :chat-group="$selectedChatGroup" wire:key="members-{{ $selectedChatGroup->id }}" />
                </div>
            </div>
        </div>
    @endif
