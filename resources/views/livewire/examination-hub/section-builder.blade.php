<section class="bg-white dark:bg-slate-900 overflow-hidden"
    ...
    <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <h2 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider"
            style="letter-spacing: 0.1em;">Sections</h2>
        <button type="button"
        ...
        @foreach($sections as $index => $section)
            <div class="p-4 border hover:bg-amber-50/40 dark:hover:bg-slate-800/40 transition-colors"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <!-- Time Limit -->
                    <div x-data="{
    sectionTime: {{ (int)($section['time_limit_minutes'] ?? 0) }},
    get examDuration() {
        const el = document.querySelector('[name=\'duration_in_minutes\']');
        return el ? (parseInt(el.value) || 0) : 0;
    },
    get exceedsExam() {
        return this.sectionTime > 0 && this.examDuration > 0 && this.sectionTime > this.examDuration;
    }
}">
                        <label
                            class="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2"
                        ...
                        <input type="number" min="1"
                               ...
                               placeholder="e.g., 30"
                               @input="sectionTime = parseInt($event.target.value) || 0"
                               :class="{ '!border-red-400 !focus:ring-red-500/20': exceedsExam }"
                               class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-none focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-slate-800 dark:text-white transition-all"
                               style="border-radius: 2px;">
                        <p x-show="exceedsExam" x-cloak
                           class="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                      d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
                                      clip-rule="evenodd"></path>
                            </svg>
                            Cannot exceed exam duration (<span x-text="examDuration"></span> min).
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    ...
                    <div>
                        ...
                        <label
                            class="inline-flex items-center px-4 py-2.5 w-full text-sm border border-slate-200 dark:border-slate-700 rounded-none focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 dark:bg-slate-800 transition-all"
                            style="border-radius: 2px;">
                            <input type="checkbox"
                ...
                @if($section['source_type'] === 'mixed')
                    <div class="mt-5 p-4 border"
                        ...
                        <p class="text-sm font-semibold text-slate-900 dark:text-white mb-3">Mixed Source
                            Configuration</p>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label
                                    class="block text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2"
                                    style="letter-spacing: 0.08em;">
                                    Database Questions
                                </label>
                        ...
                        <input type="hidden" name="sections[{{ $index }}][question_count]"
                               value="{{ ($section['database_count'] ?? 0) + ($section['ai_count'] ?? 0) + ($section['manual_count'] ?? 0) }}">
                    </div>
                @else
                    <!-- Question Count -->
                    <div class="mt-5">
                ...
                @if(in_array($section['source_type'] ?? '', ['ai', 'mixed'], true))
                    <div class="mt-5 p-4 border"
                        ...
                        <label class="block text-sm font-semibold text-slate-900 dark:text-white mb-2">Upload Document
                            for AI Generation</label>
                        ...
                        <div wire:loading wire:target="uploadedDocuments.{{ $index }}"
                ...
                @if(in_array($section['source_type'] ?? '', ['database', 'mixed'], true))
                    <div
                        class="mt-5 p-4 border border-emerald-300 rounded-sm bg-emerald-50 text-emerald-800 dark:bg-emerald-900/30 dark:border-emerald-700/50 dark:text-emerald-200">
                        <p class="text-sm font-semibold text-slate-900 dark:text-white mb-3">Topic & Subtopic Filters
                            (optional)</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Narrow down questions by specific
                            topics and subtopics</p>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Topics
                                    (optional)</label>
                ...
                <input type="hidden" name="sections[{{ $index }}][document_path]"
                       x-bind:value="hasDocument ? documentPath : ''">
                <input type="hidden" name="sections[{{ $index }}][document_name]"
                       x-bind:value="hasDocument ? documentName : ''">
