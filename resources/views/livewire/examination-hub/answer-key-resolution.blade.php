<div>
    ...
    <div class="bg-gradient-to-br from-gray-800 via-gray-900 to-gray-900 shadow-xl border-b-2 border-purple-500 mb-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-400">
                        Correct answer keys in the question bank and re-grade all affected exam submissions automatically.
                    </p>
                </div>
            ...
            <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {{-- Topic (cascades from subject) --}}
                <div>
                    <label class="block text-xs font-medium text-gray-400 mb-1">Topic</label>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div wire:loading.block wire:target="subjectId,topicId"
             class="space-y-4">
        ...
        <div wire:loading.remove wire:target="subjectId,topicId">

            @if(!$topicId)
                {{-- ── Empty state: no topic selected ─────────────────────── --}}
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    <div class="w-14 h-14 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center mb-4">
                        <svg class="w-7 h-7 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                        </svg>
                    </div>
                    <h3 class="text-base font-medium text-gray-700 dark:text-gray-300">Select a subject and topic</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Choose a subject and topic above to load the question bank for that topic.
                    </p>
                </div>

            @elseif(empty($this->questionData))
                {{-- ── Empty state: topic has no MCQ questions ────────────── --}}
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h3 class="text-base font-medium text-gray-700 dark:text-gray-300">No multiple-choice questions found</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        This topic has no MCQ entries in the question bank.
                    </p>
                </div>

            @else
                {{-- ── Alpine editor (reinitialises when topic changes via wire:key) ── --}}
                <div
                    wire:key="answer-key-editor-{{ $topicId }}"
                    x-data="answerKeyEditor(@js($this->questionData), $wire)"
                    x-init="init()"
                    @answer-key-saved.window="onSaved()"
                >
                    ...
                    <div class="mb-5 flex items-start gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded p-3 text-sm text-blue-700 dark:text-blue-300">
                        ...
                        <span>
                            Saving will update the question bank, sync corrections into all live exam questions that used these answers,
                        ...
                        <div class="flex items-center gap-2">
                            <select x-model="sortBy"
                                    class="text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-3 py-2
                                           text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-purple-500">
                        ...
                        <template x-for="(q, index) in filteredQuestions" :key="q.id">
                            <div :class="isChanged(q.id)
                                ...
                                <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                                    <div class="flex items-center gap-2">
                                        <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-700 text-xs font-semibold text-gray-600 dark:text-gray-300"
                                              x-text="index + 1"></span>
                                        <span class="text-xs text-gray-400 dark:text-gray-500" x-text="'MCQ #' + q.id"></span>
                                ...
                                <div class="p-4 space-y-4">
                                    ...
                                    <div class="prose prose-sm max-w-none dark:prose-invert text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-700/50 rounded p-3 border border-gray-100 dark:border-gray-700"
                                         x-html="renderMd(q.question) || '(no question text)'">
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-2">
                                        <template x-for="letter in optionLetters(q.option_count)" :key="letter">
                                            <div class="flex items-center gap-2">
                                                {{-- Letter badge --}}
                                                <span :class="edits[q.id] && edits[q.id].answer === letter
                                                ...
                                                <button
                                                    ...
                                                    type="button"
                                                    :class="edits[q.id] && edits[q.id].answer === letter
                                                    ...
                                                    <svg x-show="edits[q.id] && edits[q.id].answer === letter"
                                    ...
                                    <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 pt-1">
                                        ...
                                        <span class="font-semibold text-gray-700 dark:text-gray-200"
                                              x-text="'Option ' + (edits[q.id] ? edits[q.id].answer : q.answer)"></span>
                                        <template x-if="edits[q.id] && edits[q.id].answer !== q.answer">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('answerKeyEditor', (initialQuestions, wire) => ({
                            ...
                            get filteredQuestions() {
                                ...
                                list.sort((a, b) => {
                                    let av, bv;
                                    if (this.sortBy === 'id')       { av = a.id;      bv = b.id; }
                                    else if (this.sortBy === 'answer') { av = (this.edits[a.id]?.answer || a.answer); bv = (this.edits[b.id]?.answer || b.answer); }
                                    else if (this.sortBy === 'modified') { av = this.isChanged(a.id) ? 0 : 1; bv = this.isChanged(b.id) ? 0 : 1; }
                                    else                            { av = a._index; bv = b._index; }
                            ...
                            isChanged(questionId) {
                                ...
                                return orig.answer   !== edit.answer   ||
                            ...
                            async save() {
                                ...
                                this.saving = true;
                                try {
                                    // applyChanges() dispatches 'answer-key-saved' on success,
                                    // which onSaved() catches to promote edits → originals.
                                    await wire.applyChanges(diff);
                                } catch (e) {
                                    this.saving = false;
                                    console.error('Answer key save failed', e);
                                }
                            },
        }));
    });
