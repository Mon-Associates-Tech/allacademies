<div class="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200 dark:border-gray-700">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <button x-data="{ filtersOpen: @entangle('filtersOpen') }"
                ...
                class="inline-flex items-center justify-center px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors w-full sm:w-auto">
    ...
    <div x-data="{ filtersOpen: @entangle('filtersOpen') }"
         x-show="filtersOpen"
         x-collapse
         class="mb-4 p-3 sm:p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div>
                ...
                <select wire:model.live="filterShareType"
                    ...
                    <option value="academic_group">Groups</option>
                    <option value="academic_level">Levels</option>
                ...
                @foreach($shares as $index => $share)
                    <div class="p-3 sm:p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        @if($share->share_type === 'individual' && $share->sharedWithUser)
                            {{-- Individual User - Mobile Optimized --}}
                            <div class="flex flex-col sm:flex-row sm:items-start gap-3">
                                {{-- Avatar and Main Info --}}
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    <x-avatar class="h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-start justify-between gap-2">
                                            ...
                                            <button type="button"
                                                    ...
                                                    wire:confirm="Remove access for {{ $share->sharedWithUser->name }}?"
                                                    class="sm:hidden flex-shrink-0 p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                                                    title="Remove">
                                        ...
                                        <div class="flex flex-wrap items-center gap-1.5 mt-2">
                                            ...
                                            @if($share->notification_sent)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium text-green-600 dark:text-green-400">
                                ...
                                <button type="button"
                                        wire:click="removeShare({{ $share->id }}, 'individual', {{ $share->sharedWithUser->id }})"
                                        wire:confirm="Remove access for {{ $share->sharedWithUser->name }}?"
                                        class="hidden sm:block flex-shrink-0 p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                                        title="Remove access">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                        ...
                        @else
                            {{-- Group Share - Mobile Optimized --}}
                            <div class="flex flex-col sm:flex-row sm:items-start gap-3">
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white flex-shrink-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-start justify-between gap-2">
                                            ...
                                            <button type="button"
                                                    ...
                                                    wire:confirm="Remove this group share?"
                                                    class="sm:hidden flex-shrink-0 p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
    ...
    @else
        <div class="text-center py-8 sm:py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 px-4">
                ...
                @else
                    Share this note to collaborate
                @endif
            </p>
            @if($this->activeFiltersCount > 0)
                <button wire:click="clearFilters"
