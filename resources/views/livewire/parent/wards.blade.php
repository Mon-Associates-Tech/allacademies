<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div class="mb-6 lg:mb-0">
                ...
                <div class="mt-4 flex items-center space-x-6">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm">{{ $this->wards->count() }} Active Ward{{ $this->wards->count() !== 1 ?
                            's' : '' }}</span>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="w-2 h-2 bg-blue-400 rounded-full"></div>
                        <span class="text-sm">{{ $this->performanceOverview['total_assessments'] ?? 0 }} Total
                            Assessments</span>
                    </div>
                </div>
            </div>
            <div class="flex-shrink-0">
                <div class="bg-white/20 backdrop-blur-sm rounded-xl p-6 text-center">
                    <div class="text-3xl font-bold mb-1">{{ number_format($this->performanceOverview['overall_average']
                        ?? 0, 1) }}%</div>
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <button wire:click="sortBy('name')"
                        ...
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M{{ $sortDirection === 'asc' ? '8 9l4-4 4 4' : '16 15l-4 4-4-4' }}" />
                        </svg>
                        @endif
                    </button>
                </div>
    ...
    {{-- <div
        ...
        <div
            class="group border-b border-gray-200 dark:border-gray-700 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-200">
            <!-- Main Ward Row -->
            <div class="p-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="relative">
                            <div
                                class="w-12 h-12 bg-gradient-to-r from-violet-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg">
                        ...
                        <div class="min-w-0">
                            <h3 class="font-semibold text-gray-900 dark:text-gray-100 text-lg">{{ $ward->user->name }}
                            </h3>
                    ...
                    <div class="hidden md:flex items-center space-x-2">
                        ...
                        <div class="text-right">
                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
                                $ward->academicLevel->academicGroup->name ?? 'N/A' }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $ward->academicLevel->name ?? 'N/A'
                                }}</p>
                        </div>
                    </div>
                    ...
                    <!-- Actions -->
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('feepayment.form', $ward->id) }}"
                ...
                <div class="md:hidden mb-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-600 dark:text-gray-400">
                            {{ $ward->academicLevel->academicGroup->name ?? 'N/A' }} - {{ $ward->academicLevel->name ??
                            'N/A' }}
                        </span>
                    </div>
                </div>
    ...
    <div
        ...
        @forelse($this->wards as $index => $ward)
@php
// Get the active academic term (where is_current = 1)
$currentTerm = \App\Models\AcademicPeriod::where('is_current', 1)->first();
$currentTermId = $currentTerm->id ?? null;

// Fetch the matching fee structure
$fee = \App\Models\AcademicFeeStructure::where('school_id', $ward->school_id)
    ->where('academic_group_id', $ward->academic_group_id)
    ->where('academic_level_id', $ward->academic_level_id)
    ->where('current_term_id', $currentTermId)
    ->first();

$termTotalAmount = $fee->term_total_amount ?? $fee->amount ?? 0;

// Sum all payments for this student (ward) in the current term
$totalPaid = \App\Models\SchoolFee::where('student_id', $ward->id)
...
$paymentMethod = $fee->payment_method ?? 'Momo';
...
@endphp
        ...
        <div
            ...
            <div class="p-6 flex flex-col md:flex-row md:items-center md:justify-between">
                ...
                <!-- Academic Level -->
                <div class="hidden md:flex items-center space-x-2 text-right">
                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
                        $ward->academicLevel->academicGroup->name ?? 'N/A' }}</p>
                ...
                <div class="flex items-center space-x-2 mt-2 md:mt-0">
                    <a href="{{ route('feepayment.form', $ward->id) }}"
                        class="inline-flex items-center px-4 py-2 bg-violet-500 hover:bg-violet-600 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                        Pay Fees
                    </a>
                    <button wire:click="selectWard({{ $ward->id }})"
                        class="inline-flex items-center px-4 py-2 bg-violet-500 hover:bg-violet-600 text-white text-sm font-medium rounded-lg transition-colors duration-200">
            ...
            <div class="px-6 pb-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                    <div class="text-xl font-bold text-gray-900 dark:text-gray-100">{{
                        number_format($wardPerformance['average_score'], 1) }}%</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Average Score</div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                    <div class="text-xl font-bold text-gray-900 dark:text-gray-100">{{
                        $wardPerformance['assessments_count'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Tests Taken</div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                    <div class="text-xl font-bold text-green-600 dark:text-green-400">{{
                        $wardPerformance['passed_count'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">Passed</div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
        ...
        <tr class="bg-white dark:bg-gray-800">
            <td class="px-4 py-2">
                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                    {{ $ward->academicLevel->academicGroup->name ?? 'N/A' }}
                </div>
                <div class="text-xs text-gray-500 dark:text-gray-400">
                    {{ $ward->academicLevel->name ?? 'N/A' }}
                </div>
            </td>
            ...
            <td class="px-4 py-2">₵{{ number_format($remainingAmount, 2) }}</td>
            <td class="px-4 py-2">
                <span
                    class="px-3 py-1 rounded-full text-xs font-semibold 
                    @if(strtolower($feeStatus) === 'completed')
                        bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100
                    @elseif(strtolower($feeStatus) === 'part payment')
            ...
            <td class="px-4 py-2">{{ $paymentMethod }}</td>
        </tr>
    </tbody>
</table>
</div>
...
@php
    ...
    $schoolFees = \App\Models\SchoolFee::with('payer')
...
<div class="px-6 pb-6 mt-8">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <thead class="bg-gray-100 dark:bg-gray-700">
            <tr>
        ...
        <tbody>
            @forelse($schoolFees as $fee)
                @php
                    $afs = \App\Models\AcademicFeeStructure::where('school_id', $fee->school_id)
                        ->where('academic_group_id', $ward->academic_group_id)
                        ->where('academic_level_id', $ward->academic_level_id)
                        ->latest()
                        ->first();

                    $termName = $afs && $afs->current_term_id
                        ? \App\Models\AcademicPeriod::find($afs->current_term_id)?->name
                        : null;

                    $paidDate = $fee->created_at ? \Carbon\Carbon::parse($fee->created_at)->format('M d, Y') : 'N/A';
                @endphp

                <tr class="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900 transition">
                    <td class="px-4 py-2 font-medium text-gray-900 dark:text-gray-100">
                        ₵{{ number_format($fee->amount, 2) }}
                    </td>

                    <td class="px-4 py-2">{{ $fee->currency ?? 'GHS' }}</td>

                    <td class="px-4 py-2">
                        <span class="px-3 py-1 rounded-full text-xs font-semibold
                            {{ strtolower($fee->status) === 'succeeded'
                                ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100'
                                : 'bg-orange-100 text-orange-800 dark:bg-orange-700 dark:text-orange-100' }}">
                    ...
                    <td class="px-4 py-2">{{ $fee->payer?->name ?? 'N/A' }}</td>

                    <td class="px-4 py-2">{{ $termName ?? 'N/A' }}</td>

                    <td class="px-4 py-2">{{ $ward->academicLevel->academicGroup->name ?? 'N/A' }}</td>

                    <td class="px-4 py-2">{{ $ward->academicLevel->name ?? 'N/A' }}</td>

                    <td class="px-4 py-2">{{ $paidDate }}</td>
                </tr>
            @empty
                <tr>
    ...
    <div class="mt-4">
        {{ $schoolFees->links('pagination::tailwind') }}
    </div>
</div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6">
                <div class="flex items-center space-x-4 mb-6">
                    <div
                        class="w-16 h-16 bg-gradient-to-r from-violet-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-xl">
                        {{ substr($this->selectedWard->user->name, 0, 1) }}
                    </div>
                    <div>
                        <h4 class="text-xl font-semibold text-gray-800 dark:text-gray-100">{{
                            $this->selectedWard->user->name }}</h4>
                        <p class="text-gray-600 dark:text-gray-400">{{ $this->selectedWard->user->email }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{
                            $this->selectedWard->academicLevel->academicGroup->name ?? 'N/A' }} - {{
                            $selectedWard->academicLevel->name ?? 'N/A' }}</p>
                    </div>
                </div>
