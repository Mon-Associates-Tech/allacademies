        ...
        <div class="{{ $activeTab === 'book-notes' ? '' : 'hidden' }}">
            ...
            <div class="mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-start space-x-3">
                    <svg class="w-6 h-6 text-indigo-600 dark:text-indigo-400 flex-shrink-0 mt-1" fill="none"
            ...
            <div
                class="mb-8 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-gray-800 dark:to-gray-750 rounded-lg p-6 border border-indigo-100 dark:border-gray-700">
                <div class="flex items-center mb-4">
                    <svg class="w-6 h-6 text-indigo-600 dark:text-indigo-400 mr-2" fill="none" stroke="currentColor"
                         viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                    <input
                        ...
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    >
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach(\App\Models\Note::getBackgroundColors() as $key => $color)
                                <button
                                    type="button"
                                    wire:click="$set('newNoteColor', '{{ $key }}')"
                                    class="px-4 py-2 rounded-lg border-2 transition-all {{ $color['class'] }} {{ $newNoteColor === $key ? 'border-indigo-600 ring-2 ring-indigo-500' : 'border-gray-300 dark:border-gray-600' }}">
                                    {{ $color['name'] }}
                                </button>
                            @endforeach
                        </div>
                    ...
                    <div class="flex justify-end">
                        <button
                            ...
                            @click="
                                ...
                                const title = $wire.get('newNoteTitle');
                                $wire.saveNote(content, title);
                            "
            ...
            <div class="space-y-4">
                ...
                @if($this->bookNotes->isEmpty())
                    <div
                        class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 dark:border-gray-600">
                ...
                @else
                    @foreach($this->bookNotes as $note)
                        ...
                        @endphp
                        <div
                            class="border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:shadow-md transition-shadow {{ $note->getBackgroundClass() }}">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            @foreach(\App\Models\Note::getBackgroundColors() as $key => $color)
                                                <button
                                                    type="button"
                                                    wire:click="$set('editingColor', '{{ $key }}')"
                                                    class="px-4 py-2 rounded-lg border-2 transition-all {{ $color['class'] }} {{ $editingColor === $key ? 'border-indigo-600 ring-2 ring-indigo-500' : 'border-gray-300 dark:border-gray-600' }}">
                                                    {{ $color['name'] }}
                                                </button>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <button
                                            ...
                                            @click="
                                                ...
                                                const editor = tinymce.get(editorId);
                                                const content = editor ? editor.getContent() : '';
                                                const title = $wire.get('editingTitle');
                                                $wire.updateNote(content, title);
                                            "
                                            wire:loading.attr="disabled"
                                            ...
                                            <svg wire:loading class="animate-spin -ml-1 mr-2 h-4 w-4" fill="none"
                                                 viewBox="0 0 24 24">
                                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                                        stroke-width="4"></circle>
                                                <path class="opacity-75" fill="currentColor"
                            ...
                            @else
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">{{ $note->title ?? 'Untitled' }}</h3>
                                <div
                                    class="{{ $hasLongContent && !$isExpanded ? 'max-h-40 overflow-hidden relative' : '' }}">
                                    ...
                                    @if($hasLongContent && !$isExpanded)
                                        <div
                                            class="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-current to-transparent pointer-events-none opacity-30"></div>
                                    @endif
                                </div>
                                ...
                                @if($hasLongContent)
                                    <button
                                        ...
                                        <svg
                                            class="w-4 h-4 ml-1 transition-transform {{ $isExpanded ? 'rotate-180' : '' }}"
            ...
            @else
                <div class="space-y-4">
                    @foreach($this->userNotes as $note)
                        ...
                        @endphp
                        <div
                            class="bg-white dark:bg-gray-750 border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:shadow-md transition-shadow">
                            @if($editingNoteId === $note->id)
                                <div class="space-y-4">
                            ...
                            @else
                                ...
                                <div
                                    ...
                                    @if($hasLongContent && !$isExpanded)
                                        <div
                                            class="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-white dark:from-gray-750 to-transparent pointer-events-none"></div>
                                    @endif
                                </div>
