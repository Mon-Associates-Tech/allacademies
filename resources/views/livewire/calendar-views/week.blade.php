            ...
            <div class="grid grid-cols-8 border-b">
                ...
                @for($i = 0; $i < 7; $i++)
                    @php
                        ...
                        $isToday = $day->isToday();
                    @endphp
                    <div class="p-1 sm:p-2 text-center {{ $isToday ? 'bg-blue-100 dark:bg-blue-900' : '' }}">
                        <div class="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">{{ $day->format('D') }}</div>
                        <div class="text-sm sm:text-lg {{ $isToday ? 'font-bold text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-gray-100' }}">{{ $day->day }}</div>
                    </div>
                @endfor
            </div>
                    ...
                    >
                        @foreach($dayEvents as $event)
                            ...
                            <div
                                ...
                                class="absolute left-0.5 right-0.5 sm:left-1 sm:right-1 rounded p-0.5 sm:p-1 text-xs cursor-pointer overflow-hidden"
                                style="top: {{ $top }}px; height: {{ $height }}px; background-color: {{ $event->color ?: '#3b82f6' }}; color: white;"
                                title="{{ $event->title }}"
                            >
                                <div class="font-medium truncate">{{ $event->title }}</div>
                                <div class="truncate hidden sm:block">{{ $event->start_date->format('g:i A') }}</div>
                            </div>
                        @endforeach
                    </div>
                @endfor
