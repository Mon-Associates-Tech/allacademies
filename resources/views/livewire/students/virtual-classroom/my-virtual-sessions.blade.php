    ...
    <div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                View and join your scheduled virtual classroom sessions
            </p>
        </div>
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            @forelse($participants as $participant)
                @php $session = $participant->virtualSession; @endphp
                <div class="overflow-hidden transition-shadow bg-white rounded-lg shadow hover:shadow-lg dark:bg-gray-800">
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <div class="flex items-center">
                                ...
                                {{ $session->scheduled_start->format('g:i A') }} ({{ $session->duration_minutes }} min)
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                @if($participant->has_joined)
                                    <span class="text-xs text-gray-500 dark:text-gray-400">
                                        Joined {{ $participant->join_count }} time(s)
                                    </span>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700">
                        @if($session->status === 'live')
                            <button wire:click="joinSession({{ $participant->id }})"
                                    class="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700">
                        ...
                        @elseif($session->status === 'ended')
                            @if($session->hasRecordings())
                                <a href="{{ route('students.classroom.recordings') }}"
                                   class="w-full inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-violet-700 bg-violet-100 rounded-lg hover:bg-violet-200 dark:bg-violet-900/20 dark:text-violet-400 dark:hover:bg-violet-900/30">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="col-span-full">
                    <div class="py-12 text-center bg-white rounded-lg shadow dark:bg-gray-800">
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            @if($view === 'upcoming')
                                You don't have any upcoming virtual sessions.
                            @else
