<div class="container mx-auto px-4 py-6">
    ...
    @if ($showDetails && $selectedAssignment)
        <div class="fixed inset-0 z-50 overflow-y-auto" wire:click="closeDetails">
                ...
                <div class="inline-block w-full max-w-4xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-xl rounded-lg" wire:click.stop>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $selectedAssignment->academicSubject->name ?? 'Unknown Subject' }}
                                    </p>
                                </div>
                            </div>
                            <button
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-3">Assignment Details</h4>
                                    <div class="space-y-3">
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-500 dark:text-gray-400">Duration</span>
                                            <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $selectedAssignment->duration_in_minutes }} min</span>
                                        </div>
                                        @if ($selectedAssignment->total_marks)
                                            <div class="flex justify-between">
                                                <span class="text-sm text-gray-500 dark:text-gray-400">Total Marks</span>
                                                <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $selectedAssignment->total_marks }}</span>
                                            </div>
                                        @endif
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-500 dark:text-gray-400">Teacher</span>
                                            <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $selectedAssignment->teacher->user->name ?? 'Unknown' }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Start Date</div>
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $selectedAssignment->starts_at->format('M j, Y g:i A') }}</div>
                                        </div>
                                        <div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">End Date</div>
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $selectedAssignment->ends_at->format('M j, Y g:i A') }}</div>
                                        </div>
                                        <div>
                                            ...
                                            @endphp
                                            <div class="text-sm font-medium {{ $isOverdue ? 'text-red-600 dark:text-red-400' : ($timeLeft < 24 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-900 dark:text-gray-100') }}">
                                                @if ($isOverdue)
                                                    Overdue
                                                @elseif ($timeLeft < 1)
                                                    Less than 1 hour
                                ...
                                @if ($progress > 0)
                                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-3">Progress</h4>
                                        <div class="space-y-2">
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600">
                        <div class="flex justify-end space-x-3">
                            <button
                                wire:click="closeDetails"
                                class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
                            >
                                Close
                            </button>
                            @if ($this->getAssignmentStatus($selectedAssignment) !== 'completed' && $selectedAssignment->ends_at > now())
                                <button
                                    wire:click="startAssignment({{ $selectedAssignment->id }})"
                                    class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-violet-500 to-purple-600 rounded-lg hover:from-violet-600 hover:to-purple-700 transition-all duration-200 shadow-sm"
                                >
                                    @if ($this->getAssignmentStatus($selectedAssignment) === 'in_progress')
                                        Continue Assignment
                                    @else
                                        Start Assignment
                                    @endif
