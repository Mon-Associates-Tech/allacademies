...
<template x-teleport="body">
    <div x-show="{{ $show ?? 'showMessageModal' }}"
         ...
         @click.self="{{ $close ?? 'showMessageModal = false' }}">
        <div class="bg-white dark:bg-slate-900 w-full max-w-md overflow-hidden"
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                <h3 class="font-bold text-slate-900 dark:text-white">{{ $title ?? 'Send Message' }}</h3>
                <p class="text-sm text-slate-500 mt-1" x-text="{{ $subtitle ?? \"'To: ' + (selectedParticipant?.participant_name || '')\" }}"></p>
            </div>
            <div class="px-6 py-4">
                <textarea x-model="{{ $model ?? 'messageText' }}"
                          rows="3"
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                <button @click="{{ $close ?? 'showMessageModal = false' }}"
                ...
                <button @click="{{ $action ?? 'sendMessage()' }}"
                        :disabled="{{ $disabled ?? '!messageText.trim() || actionLoading' }}"
