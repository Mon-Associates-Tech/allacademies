...
<template x-teleport="body">
    <div x-show="{{ $show ?? 'showTerminateModal' }}"
         ...
         @click.self="{{ $close ?? 'showTerminateModal = false' }}">
        <div class="bg-white dark:bg-slate-900 w-full max-w-md overflow-hidden"
            ...
            <div class="px-6 py-4">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ $label ?? 'Reason' }} <span class="text-red-500">*</span></label>
                <textarea x-model="{{ $model ?? 'terminateReason' }}"
                          rows="3"
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                <button @click="{{ $close ?? 'showTerminateModal = false' }}"
                ...
                <button @click="{{ $action ?? 'executeTerminate()' }}"
                        :disabled="{{ $disabled ?? '!terminateReason.trim() || actionLoading' }}"
