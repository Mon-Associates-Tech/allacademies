...
<template x-teleport="body">
    <div x-show="{{ $show ?? 'showExtendTimeModal' }}"
         ...
         @click.self="{{ $close ?? 'showExtendTimeModal = false' }}">
        <div class="bg-white dark:bg-slate-900 w-full max-w-md overflow-hidden"
            ...
            <div class="px-6 py-4">
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ $label ?? 'Minutes to add' }}</label>
                <input x-model="{{ $model ?? 'extendMinutes' }}" type="number" min="1" max="480"
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                <button @click="{{ $close ?? 'showExtendTimeModal = false' }}"
                ...
                <button @click="{{ $action ?? 'extendTime()' }}"
                        :disabled="{{ $disabled ?? '!extendMinutes || extendMinutes < 1 || actionLoading' }}"
