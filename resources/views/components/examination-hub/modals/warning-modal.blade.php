...
<template x-teleport="body">
    <div x-show="{{ $show ?? 'showWarningModal' }}"
         ...
         @click.self="{{ $close ?? 'showWarningModal = false' }}">
        <div class="bg-white dark:bg-slate-900 w-full max-w-md overflow-hidden"
            ...
            <div class="px-6 py-4">
                <textarea x-model="{{ $model ?? 'warningText' }}"
                          rows="3"
                          placeholder="{{ $placeholder ?? 'Enter warning message...' }}"
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                <button @click="{{ $close ?? 'showWarningModal = false' }}"
                ...
                <button @click="{{ $action ?? 'sendWarning()' }}"
                        :disabled="{{ $disabled ?? '!warningText.trim() || actionLoading' }}"
