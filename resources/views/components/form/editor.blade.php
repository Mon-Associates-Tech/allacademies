@props(['name', 'label' => null, 'full' => false, 'value' => ''])
...
<div class="space-y-1">
    ...
    <div x-data="{ preview: false, down: '{{ $down }}', up: '{{ $up }}' }"
        ...
        <div class="bg-white border-x border-t border-gray-300">
        ...
        <textarea x-model.lazy="down" x-show="!preview" id="{{ $name }}_down" name="{{ $name }}[down]" @class([
            'block px-4 py-2 text-gray-700 focus:outline-none focus:border-gray-700 bg-white border border-gray-300',
            'w-full' => $full
            ])></textarea>

        <div x-html="up" x-show="preview" @class([
            'block px-4 py-2 text-gray-700 focus:outline-none focus:border-gray-700 bg-white border border-gray-300',
            'w-full' => $full
        ])></div>
