        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <x-academic-header>
                ...
                <x-slot name="headerContent">
                    <div class="flex items-center space-x-4">
                        <div>
                            <h1 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">Multiple Choice Questions</h1>
                ...
                @can('moderate')
                    <x-slot name="headerActions">
                        <div class="flex items-center gap-2">
                            @if($multipleChoiceQuestions->total() > 0)
                            <x-link.secondary :to="route('multiple-choice-questions.bulk-edit', ['academic_topic' => $academicTopic, 'academic_subject' => getRouteParameter('academic_subject'), 'academic_level' => getRouteParameter('academic_level'), 'academic_group' => getRouteParameter('academic_group')])"
                            ...
                            @endif
                            <x-link.primary :to="route('multiple-choice-questions.create', ['academic_topic' => $academicTopic, 'academic_subject' => getRouteParameter('academic_subject'), 'academic_level' => getRouteParameter('academic_level'), 'academic_group' => getRouteParameter('academic_group')])"
                                            class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 hover:scale-105">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if ($multipleChoiceQuestions->count())
                <div class="border-t border-gray-200 dark:border-gray-700">
                                ...
                                @foreach($multipleChoiceQuestions as $multipleChoiceQuestion)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-normal">
                                            <x-markdown-viewer :content="$multipleChoiceQuestion->question" display="true" class="prose dark:prose-invert max-w-none" />
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end space-x-2">
                                                <x-link.secondary :to="route('multiple-choice-questions.show', ['multiple_choice_question' => $multipleChoiceQuestion, 'academic_subject' => getRouteParameter('academic_subject'), 'academic_topic' => getRouteParameter('academic_topic'), 'academic_level' => getRouteParameter('academic_level'), 'academic_group' => getRouteParameter('academic_group')])"
                    ...
                    @if($multipleChoiceQuestions->hasPages())
            ...
            @else
                <div class="border-t border-gray-200 dark:border-gray-700 text-center py-16 px-6">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
                        Get started by creating your first multiple choice question for <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $academicTopic->name }}</span>.
                    </p>
                    @can('moderate')
                        <div class="space-y-4">
                            <x-link.primary :to="route('multiple-choice-questions.create', ['academic_topic' => getRouteParameter('academic_topic'), 'academic_subject' => getRouteParameter('academic_subject'), 'academic_level' => getRouteParameter('academic_level'), 'academic_group' => getRouteParameter('academic_group')])"
