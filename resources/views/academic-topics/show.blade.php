<x-auth title="Academic Topic Details">
    ...
    <x-detail>
        ...
        <x-detail.data label="Academic Subject">
            <x-anchor to="{{ route('academic-subjects.show', ['academic_subject' => $academicTopic->academicSubject]) }}">
                {{ $academicTopic->academicSubject->name }}
            </x-anchor>
        </x-detail.data>

        <x-detail.data label="Multiple Choice Questions">
            <x-anchor to="{{ route('academic-topics.multiple-choice-questions.index', ['academic_topic' => $academicTopic]) }}">
                {{ $academicTopic->multiple_choice_questions_count }} multiple choice {{ Str::plural('question', $academicTopic->multiple_choice_questions_count) }}
            </x-anchor>
        </x-detail.data>

        <x-detail.data label="True Or False Questions">
            <x-anchor to="{{ route('academic-topics.true-or-false-questions.index', ['academic_topic' => $academicTopic]) }}">
                {{ $academicTopic->true_or_false_questions_count }} true or false {{ Str::plural('question', $academicTopic->true_or_false_questions_count) }}
            </x-anchor>
        </x-detail.data>

        <x-detail.data label="Essay Questions">
            <x-anchor to="{{ route('academic-topics.essay-questions.index', ['academic_topic' => $academicTopic]) }}">
                {{ $academicTopic->essay_questions_count }} essay {{ Str::plural('question', $academicTopic->essay_questions_count) }}
            </x-anchor>
        </x-detail.data>

        @can('administrate')
        <x-slot name="action">
            <x-button.secondary type="buttton" x-data="{}" x-on:click="$store.deleteForm.show('Danger', 'Are you sure you want to delete {{ $academicTopic->name }}', '{{ route('academic-topics.destroy', ['academic_topic' => $academicTopic]) }}')">Delete Academic Topic</x-button.secondary>
            <x-link.primary :to="route('academic-topics.edit', ['academic_topic' => $academicTopic])">Edit Academic Topic</x-link.primary>
        </x-slot>
        @endcan
    </x-detail>
</x-auth>