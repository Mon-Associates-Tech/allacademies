<x-bookshop::layouts.staff :title="'New Restock Request - BookShop'">
    <h1 class="text-xl font-bold text-slate-900 dark:text-white" style="font-family: 'Georgia', serif;">Request
        Stock</h1>
    ...
    <div class="bg-white dark:bg-slate-900 p-6 max-w-2xl"
        ...
        <form method="POST" action="{{ route('bookshop.staff.restock-requests.store') }}" class="space-y-6"
            ...
            <div id="book-rows-container" class="space-y-4">
                ...
                @for($i = 0; $i < $initialRows; $i++)
                    <div
                        class="grid sm:grid-cols-5 gap-3 items-end pb-4 border-b border-slate-100 dark:border-slate-800 book-row">
                        ...
                        <div class="sm:col-span-2 relative">
                            ...
                            <input type="number" name="items[{{ $i }}][quantity]" min="1"
                                   ...
                                   class="w-full px-3 py-2.5 text-sm border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                                   style="border-radius: 2px;">

                            <button type="button"
                                    class="remove-row absolute -top-2 -right-2 text-slate-400 hover:text-red-500 p-1 transition-colors"
                                    title="Remove book" style="display: {{ $initialRows === 1 ? 'none' : 'block' }}">
            ...
            <a href="{{ route('bookshop.staff.books.index') }}"
               class="inline-block ml-2 text-sm text-slate-500 dark:text-slate-400 underline">
            ...
            <div>
                <label
                    ...
                    style="letter-spacing: 0.08em;">Notes (optional, applies to the whole request)</label>
                <textarea name="notes" rows="3"
                          class="w-full px-4 py-2.5 text-sm border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                          style="border-radius: 2px;">{{ old('notes') }}</textarea>
            </div>
    ...
    <!-- Template for dynamically added rows. Mirrors the markup rendered by
         the book-picker component (including the -stock-info placeholder) so
    ...
    <template id="book-row-template">
        <div class="grid sm:grid-cols-5 gap-3 items-end pb-4 border-b border-slate-100 dark:border-slate-800 book-row">
            <div class="sm:col-span-3">
                <label for="restock-item-__INDEX__-search"
                       class="block text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2"
                ...
                <div data-book-picker="true" data-prefix="restock-item-__INDEX__" class="relative">
                    <input type="text" id="restock-item-__INDEX__-search" autocomplete="off"
                           ...
                           style="border-radius: 2px;">
                    <input type="hidden" name="items[__INDEX__][book_id]" id="restock-item-__INDEX__-value"
                           data-book-picker-value>
    ...
    @push('scripts')
        <script>
            ...
            // itself lives in the book-picker component's own
