<x-bookshop::layouts.customer :title="'Catalog - BookShop'">
    ...
    @else
        <p class="text-sm text-slate-500 dark:text-slate-400">Orders are served by <strong>{{ $branch->name }}</strong> ({{ $branch->city }}).</p>
    @endif
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        @forelse($books as $book)
            ...
            <a href="{{ route('bookshop.shop.books.show', $book) }}"
