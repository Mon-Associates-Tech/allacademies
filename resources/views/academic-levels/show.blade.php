<x-layouts.app title="Academic Level Details" page-name="Academic Details" :has-action="false">
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Academic Subjects Card - Spans 2 columns -->
            <div class="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
                ...
                @if($academicLevel->academicSubjects && $academicLevel->academicSubjects->count() > 0)
                    <div class="mt-4">
                        <h4 class="text-sm font-medium text-gray-700 mb-3">Available Subjects:</h4>
                        <div class="space-y-2">
                                ...
                                <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if(isset($subject->topics_count))
                                            <span class="text-xs text-gray-500">{{ $subject->topics_count }} topics</span>
                                        @endif
                ...
                @else
                    <div class="mt-4">
                        <p class="text-sm text-gray-500">No subjects available for this level.</p>
                        @can('administrate')
                            <x-anchor to="{{ route('academic-subjects.create', ['academic_level' => $academicLevel, 'academic_group' => $academicLevel->academicGroup]) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium mt-2 inline-block">
                                Add first subject →
                            </x-anchor>
                        @endcan
                    </div>
            ...
            <!-- Right Column - Students and Teachers stacked -->
            <div class="space-y-6">
