<x-auth title="{{$academic_subtopic->name}} Details" :action-link-text="'HHHH'">
    <x-slot name="breadcrumb">
        <x-breadcrumb :paths="[
            ...
            'Subtopics' => route('academic-topics.subtopics.index', ['academic_topic' => $academicTopic]),
        ]"/>
    </x-slot>

    <x-detail>
        <x-detail.data label="Name">{{ $academic_subtopic->name }}</x-detail.data>

        <x-detail.data label="Academic Topic">
            <x-anchor
                to="{{ route('academic-topics.show', ['academic_topic' => $academicTopic]) }}">
                {{ $academicTopic->name }}
            </x-anchor>
        </x-detail.data>
        ...
        @can('administrate')
            <x-slot name="action">
                <x-button.secondary type="buttton" x-data="{}"
