...
class ViewPublicAssignmentResults extends Component
{
    ...
    public function boot(
        PublicAssignmentGradingService $gradingService,
        ParticipantVerificationService $verificationService
    ): void {
    ...
    {
        $submission = PublicAssignmentSubmission::with(['participant', 'proctoringSession'])
            ->where('public_assignment_id', $this->assignment->id)
            ->find($submissionId);
    ...
    {
        $query = PublicAssignmentSubmission::where('public_assignment_id', $this->assignment->id)
            ->with(['participant', 'proctoringSession']);

        // Search filter - search by participant name/email
        if (! empty($this->search)) {
            $query->where(function ($q) {
                // For PublicAssignmentParticipant
                $q->whereHasMorph('participant', ['App\Models\PublicAssignmentParticipant'], function ($query) {
