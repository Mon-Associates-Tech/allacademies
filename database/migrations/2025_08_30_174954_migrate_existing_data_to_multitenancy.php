...
//        $this->migrateAcademicLevelAssignments($schoolId);
    ...
    private function migrateAcademicLevelAssignments($schoolId): void
    {
        ...
        foreach ($existingLevels as $level) {
            ...
            $schoolAcademicGroup = DB::table('school_academic_groups')
                ->where('school_id', $schoolId)
            ...
            if ($schoolAcademicGroup) {
                ...
                $exists = DB::table('school_academic_levels')
                    ->where('school_id', $schoolId)
                    ->where('academic_level_id', $level->id)
                    ->exists();

                if (!$exists) {
                    DB::table('school_academic_levels')->insert([
                        'school_id' => $schoolId,
                        'academic_level_id' => $level->id,
                        'school_academic_group_id' => $schoolAcademicGroup->id,
                        'is_active' => true,
